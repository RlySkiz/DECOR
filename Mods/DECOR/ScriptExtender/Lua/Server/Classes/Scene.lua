...
function Scene:FindSceneByEntity(entityToSearch)
    ...
    -- _P("[DECOR][Scene.lua] - Scene:FindSceneByEntity - Entity not found in any existing scenes! This shouldn't happen anymore. Contact mod author about what you did.")
end
...
function Scene:CancelAllSoundTimers()
    ...
    if #self.timerHandles > 0 then
        _P("[DECOR][Scene.lua] - Scene:CancelAllSoundTimers() - This shouldn't happen anymore, please contact mod author about timerHandles being buggy again")
    end
end
...
initialize = function(self)
    ...
    Ext.ModEvents.DECOR.SceneInit:Throw({self})
    ...
    Ext.ModEvents.DECOR.SceneCreated:Throw({self})
end
...
function Scene:MoveSceneToLocation(entity, newLocation)
    ...
    Ext.ModEvents.DECOR.SceneMove:Throw({scene = scene, oldLoca = oldLocation, newLoca = newLocation})
end
...
function Scene:Destroy()
    ...
    Ext.ModEvents.DECOR.SceneDestroyed:Throw({self})
