...
function Actor:new(parent)
    local instance       = setmetatable({
        ...
        autoErection     = Ext.Entity.Get(parent).Vars.DECOR_AutoErection == 1,
...
function Actor:CopyEntityAppearanceOverrides()
    if Entity:TryCopyEntityComponent(self.parent, self.uuid, "AppearanceOverride") then
        ...
        elseif not self.uuid.GameObjectVisual then
            _P("[DECOR][Actor.lua] Trying to create Actor for entity without GameObjectVisual Component.")
            _P("[DECOR][Actor.lua] This can happen with some scenery NPC's.")
            _P("[DECOR][Actor.lua] Safeguards have been put in place, nothing will break. Please end the Scene and choose another target.")
        end
    end
end
...
--- Dresses an actor based on which equipment/armour (vanity slots) have been saved on it (DECOR_StrippingBlock)
function Actor:DressActor()
    ...
    Ext.ModEvents.DECOR.ActorDressed:Throw({uuid = self.uuid, eq = self.equipment})
...
initialize = function(self)
    Ext.ModEvents.DECOR.ActorInit:Throw({self.uuid})
    
    Osi.SetDetached(self.uuid, 1)
    Osi.ApplyStatus(self.uuid, "DECOR_SEXACTOR", -1) -- Marks them for SweatySex and IdleExpressions - TODO: Change those mods to just get an entities actor and do their thing without the boost
    ...
    Ext.ModEvents.DECOR.ActorCreated:Throw({self})
end