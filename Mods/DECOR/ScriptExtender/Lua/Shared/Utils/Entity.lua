...
local function resolveEntityArg(entityArg)
    if entityArg and type(entityArg) == "string" then
        ...
        if not e then
            -- _P("[DECOR][Entity.lua] resolveEntityArg: failed resolve entity from string '" .. entityArg .. "'")
        end
        return e
    end
    return entityArg
...
function Entity:TryCopyEntityComponent(uuid_1, uuid_2, componentName)
    ...
    if componentName == "ServerItem" then
        ...
        if serializeResult then
            _P("[DECOR][Entity.lua] - TryCopyEntityComponent, with component: " .. componentName)
            _P("[DECOR][Entity.lua] - Serialization fail")
            _P("[DECOR][Entity.lua] - Result: " .. serializeResult)
            return false
        end
    end
